...
class _NavigationScreenState extends State<NavigationScreen> {
  ...
  final LatLng _currentLocation = LatLng(37.5642, 126.9742);
  ...
  Future<void> _launchGoogleMapsNavigation() async {
    try {
      ...
      try {
        currentLocation = await _getCurrentUserLocation();
      } catch (e) {
        currentLocation = _currentLocation;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('현재 위치를 가져올 수 없습니다. 기본 위치를 사용합니다.')),
        );
      }

      final double hospitalLat = double.tryParse(widget.hospital['latitude']?.toString() ?? '') ?? _destinationLocation.latitude;
      final double hospitalLng = double.tryParse(widget.hospital['longitude']?.toString() ?? '') ?? _destinationLocation.longitude;

      final Uri googleMapsUrl = Uri.parse(
        'https://www.google.com/maps/dir/?api=1&origin=${currentLocation.latitude},${currentLocation.longitude}'
        '&destination=$hospitalLat,$hospitalLng&travelmode=driving'
      );

      bool shouldLaunch = await _showNavigationConfirmDialog();

      if (shouldLaunch && await canLaunchUrl(googleMapsUrl)) {
        await launchUrl(googleMapsUrl, mode: LaunchMode.externalApplication);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('내비게이션을 시작할 수 없습니다.')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<bool> _showNavigationConfirmDialog() async {
    return await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('내비게이션 시작'),
        content: Text('Google Maps 앱으로 이동하여 실시간 내비게이션을 시작하시겠습니까?'),
        actions: [
          TextButton(
  ...
  void _startMapboxNavigation() async {
    try {
      await dotenv.load();
